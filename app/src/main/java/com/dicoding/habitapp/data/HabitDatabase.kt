    ...
    companion object {
        ...
        fun getInstance(context: Context): HabitDatabase {
            return synchronized(this) {
                val roomBuilder = Room.databaseBuilder(context, HabitDatabase::class.java, "habit").build()

                INSTANCE = roomBuilder

                val getPreferences = context.getSharedPreferences("getPreferences", Context.MODE_PRIVATE)
                val loader = getPreferences.getBoolean("loader", false)

                if (!loader) {
                    getPreferences.edit().putBoolean("loader", true).apply()
                    runBlocking {
                        withContext(Dispatchers.IO) {
                            fillWithStartingData(context, roomBuilder.habitDao())
                        }
                    }
                }
