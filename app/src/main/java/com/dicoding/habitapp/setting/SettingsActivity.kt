    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            listPreference?.setOnPreferenceChangeListener { preference, newValue ->
                ...
                if (value == getString(R.string.pref_dark_follow_system)) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) updateTheme(DarkMode.FOLLOW_SYSTEM.value)

                    else updateTheme(DarkMode.ON.value)
                } else
                    if (value == getString(R.string.pref_dark_off)) updateTheme(DarkMode.OFF.value)
                else updateTheme(DarkMode.ON.value)
